...
export function PostCard(props: POSTS_QUERYResult[0]) {
  const { title, author, mainImage, date, categories, slug } = props;

  return (
    <Link className="group" href={`/blogs/${slug!}`}>
      <article className="flex flex-col-reverse gap-4 md:grid md:grid-cols-12 md:gap-0">
        ...
        <div className="md:col-span-5 md:w-full">
          <h2 className="text-2xl text-pretty font-semibold  group-hover:text-pink-600 transition-colors relative">
